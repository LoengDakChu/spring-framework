...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected ConfigurableEnvironment createEnvironment() {
		// 创建StandardEnvironment对象，会先调用父类AbstractEnvironment无参构造方法
	...
	/**
	 * 将给定事件发布到所有监听器。 注意:侦听器在MessageSource之后被初始化，以便能够在侦听器实现中访问它。因此，MessageSource实现不能发布事件。
	 *
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 初始化BeanFactory，并解析配置吗，解析组装BeanDefinition
			...
			try {
				...
				// 调用所有注册的BeanFactoryPostProcessor的相关方法
				// 区分BeanPostProcessor(Bean后置处理器)和BeanFactoryPostProcessor(Bean工厂后置处理器)
	...
	/**
	 * 实例化和注册所有的BeanPostProcessor bean，如果给出，请遵守显式的顺序。 必须在应用程序bean的任何实例化之前调用
	...
	protected BeanFactory getInternalParentBeanFactory() {
		// 如果父类是 ConfigurableApplicationContext 则拿 ConfigurableApplicationContext 的bean工厂
		// 否者直接拿父类
